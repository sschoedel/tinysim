...
class DoublePendulum():
    ...
    def draw(self, pixel_scale_func):
        self.artist.draw(self.calculate_lines_start_end(), pixel_scale_func)


class SingleStick():
    def __init__(self):
        # self.state = np.array([0, 0, 0, 1, 5, 3]) # x, y, theta, x dot, y dot, theta dot
        self.state = np.array([0, 0, np.pi/2, 0, 0, 0]) # x, y, theta, x dot, y dot, theta dot
        self.nq = 3
        self.nv = 3
        self.nu = self.nq
        self.m = 1.0
